...
## 7. 日志功能：
   ...
   aop方式实现日志功能
   日志文件每小时一生成的滚动文件
   每天0点将前一日的小时级日志文件移动到日志目录的bak/下,进行合并压缩备份
   ✅ 已完成
   - ✅ 已使用 AOP 方式实现日志功能（LogAspect）
   - ✅ 已使用 @LogRecord 注解记录操作
   - ✅ 已记录操作时间、操作类型和操作内容（包含URL、HTTP方法、类方法、参数、返回结果）
   - ✅ 已为查询、更新、删除、分页查询、按部门查询等操作添加日志注解
   - ✅ 日志文件滚动：已配置每小时滚动（自定义 logback-spring.xml），格式为 hr-app.yyyy-MM-dd-HH.log
   - ✅ 压缩功能：每天0点定时压缩备份
   - ✅ 每天0点移动到bak/并压缩：已实现（通过定时任务 LogBackupTask）

## 8. 添加Swagger文档接口
   ✅ 已完成
   - 已引入 Swagger/OpenAPI 依赖（springdoc-openapi-starter-webmvc-ui）
   - 已在控制器中使用 @Tag 和 @Operation 注解
   - 已在 DeptController 和 EmpController 中添加文档注解
